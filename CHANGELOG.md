...
- **`odoo_run_document_action` tool schema can no longer drift from
  the security map — and it had.** The tool description, the `model`
  argument description, and the `action` enum in `tools.py` were three
  ...
  resolver enforced the real one. That is exactly what happened in
  v0.20.0: `mrp.production`, `account.payment`, `hr.leave` and
  `hr.expense.sheet` gained a `cancel` row in the map, and the tool
  schema kept advertising the original four models, so an agent
  reading the schema could not know those actions existed. The schema
  now derives from the map
