...
class OdooClient:
    ...
    def _enforce_admin_refusal(self) -> None:
        ...
        raise OdooAuthError(
            ...
            f"MCP relies on.\n"
            f"\n"
            f"Recommended fix — create a dedicated non-admin Odoo user:\n"
            f"  1. In Odoo → Settings → Users & Companies → Users → New.\n"
            f"  2. Name it something like 'MCP Agent' and give ONLY the\n"
            f"     functional groups the agent actually needs — typically\n"
            f"     Sales (User), CRM (User), Inventory (User), Accounting\n"
            f"     (Billing) if invoicing is in scope. DO NOT grant\n"
            f"     'Settings' / 'Administration: Settings' or any\n"
            f"     'Administration' group — those re-introduce the\n"
            f"     record-rule bypass we're trying to keep.\n"
            f"  3. Open the new user → Account Security → New API Key.\n"
            f"  4. Run 'odoo-mcp setup --rotate-key {self._instance.name}'\n"
            f"     and paste the new key.\n"
            f"\n"
            f"Opt-out (NOT recommended — you lose the record-rule guard\n"
            f"entirely; one prompt-injection can then write anywhere):\n"
            f"set 'refuse_admin_on_production = false' in the\n"
            f"[instances.{self._instance.name}] TOML section. Only do\n"
            f"this for integration test rigs or other deliberately\n"
            f"non-user-facing environments."
        )
