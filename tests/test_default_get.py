...
def test_default_get_rejects_denied_model(make_app: Callable[..., Any]) -> None:
    ...
    payload = _call(
        Dispatcher(app),
        # res.users is readable through a hard field whitelist since v0.13;
        # ir.config_parameter is still on MODEL_DENYLIST outright.
        {"instance": "dev", "model": "ir.config_parameter", "fields": ["key"]},
    )
    assert payload["ok"] is False
